...
1. `pnpm build:lib` 通过。
2. `examples/vue`（`pnpm dev`）逐项冒烟：brush / eraser / image 导入 + 变换 / selection / clear / 缩放 / undo / redo / download / extract。
3. `examples/react`（`pnpm build:react` 或其 dev）至少能加载 + 画线。
4. `site`（`pnpm dev:site`）首页与 `/shodo` 能打开；`/shodo` 若因移植未完报错，**记录**而非修复。
