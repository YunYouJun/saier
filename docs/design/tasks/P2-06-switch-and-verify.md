...
- [ ] tile 后端下 P1-09 三断言仍全绿（行为 parity）。
- [ ] **4096×4096** 画布连续涂抹：内存平稳（不随笔数单调上升），稀疏 tile 生效。
- [ ] 撤销只恢复脏 tile（数量断言，承 P2-03）。
- [ ] 一帧内多 dab 合并为一次上传（承 P2-05）。
- [ ] `pnpm build` / `typecheck` / `lint` / `test` 全绿；demo 冒烟通过。
