...
**遗留**:① **蒙版**显示与验收随 [P6-04](./P6-04-layer-mask) 阻塞延后。

**已修**:② 浏览器测试套的**资源型 flake** —— `vitest.config` 给 browser project 设 `fileParallelism: false` + 30s timeout(浏览器 spec 串行跑,消除并发抢 GPU / 浏览器资源),连跑 3× 稳定 851/851 绿(约 30s/次);node project 仍并行。
